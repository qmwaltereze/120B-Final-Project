...
//unsigned char c[8] = {0x0E,0x01,0x01, 0x1F, 0x1b,0x1B,0x1,0x0E};
...
int DoorBell(int state){
	...
	unsigned char A2 = ~(PINA & 0x03)>>2;
	unsigned char A3 = ~(PINA & 0x04)>>3;
	unsigned char A4 = ~(PINA & 0x05)>>4;
	unsigned char A5 = ~(PINA & 0x06)>>5;
	unsigned char A6 = ~(PINA & 0x07)>>6;
...
// Task 4 (toggleLED0SM)
task5.state = light;//Task initial state.
task5.period = 300;//Task Period.
task5.elapsedTime = task5.period;//Task current elapsed time.
task5.TickFct = &Leds;//Function pointer for the tick.*/
// Task 3 (toggleLED1SM)
...
task3.TickFct = &OpenClose; // Function pointer for the tick.
// Task 4 (displaySM)
